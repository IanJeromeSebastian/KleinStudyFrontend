...
const QuizCard = ({ quiz, onDelete }) => {
    return (
        <div className='group relative bg-white/80 backdrop-blur-xl border-2 border-slate-200 hover:border-[#FFC84D] rounded-2xl p-4 transition-all duration-200 hover:shadow-lg hover:shadow-[#FFC84D]/10 flex flex-col justify-between'>
            <button
                onClick={(e) => {
                    e.stopPropagation();
                    onDelete(quiz);
            ...
            <div className='space-y-4'>
                <div className='inline-flex items-center gap-1.5 py-1 rounded-lg text-xs font-semibold'>
                    <div className='flex items-center gap-1.5 bg-[#FFEBB7] border border-[#FFC84D] rounded-lg px-3 py-1'>
            ...
            <div className="mt-2 pt-4 border-t border-slate-100">
                {quiz?.userAnswers?.length > 0 ? (
                    ...
                    <Link to={`/quizzes/${quiz._id}`}>
                        <button className="group/btn relative w-full h-full bg-linear-to-br from-[#FFC84D] to-[#FFAE00] hover:from-[#FFAE00] hover:to-[#FFAE00] font-semibold text-sm rounded-xl transition-all duration-200 shadow-lg shadow-[#FFAE00]/25 active:scale-95 overflow-hidden">
