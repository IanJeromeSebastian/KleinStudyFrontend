...
const QuizResultPage = () => {
  ...
  const { data, results: detailedResults } = results;
  ...
  return (
    <div className='max-w-5xl mx-auto'>
      <div className='mb-6'>
        <Link
          to={`/documents/${data.document._id}`}
          className='group inline-flex items-center gap-2 text-sm font-medium text-slate-600 hover:text-[#E69D00] transition-colors duration-200'>
      ...
      <PageHeader title={`${data.title || 'Quiz'} Results`} />

      <div className='bg-white/80 backdrop-blur-xl border-2 border-slate-300 rounded-2xl shadow-xl shadow-slate-200/50 p-8 mb-8'>
        <div className='text-center space-y-6'>
          <div className='inline-flex items-center justify-center w-15 h-15 rounded-2xl bg-linear-to-br from-[#FFEBB7] to-[#FFDD8A] shadow-lg shadow-[#FFAE00]/25'>
            <Trophy className='w-7 h-7 text-[#E69D00]' strokeWidth={2} />
          </div>
          ...
          <div className='flex items-center justify-center gap-4 pt-4'>
            ...
            <div className='flex items-center gap-2 px-4 py-2 bg-[#FFF4D6] border border-[#FFDD8A] rounded-xl'>
              <CheckCircle2 className='w-4 h-4 text-[#E69D00]' strokeWidth={2} />
              <span className='text-sm font-semibold text-[#B27A00]'>
            ...
            <div
              ...
              <div className='flex items-center justify-between gap-4 mb-3'>
                <div className='flex-1'>
                  <div className='inline-flex items-center gap-2 px-3 bg-slate-50 border border-slate-200 rounded-lg mb-3'>
                ...
                ${isCorrect
                    ? 'bg-[#FFEBB7] border border-[#FFDD8A]'
                    : 'bg-rose-50 border-2 border-rose-200'
                  }`}>
                  {isCorrect ? (
                    <CheckCircle2 className='w-5 h-5 text-[#E69D00]' strokeWidth={2.5} />
                  ) : (
              ...
              <div className='space-y-3 mb-4'>
                {result.options.map((option, optIndex) => {
                  ...
                  return (
                    ...
                    ${isCorrectOption
                          ? 'bg-[#FFEBB7] border-[#FFC84D] shadow-lg shadow-[#FFAE00]'
                          : isWrongAnswer
                      ...
                      <div className='flex items-center justify-between gap-3'>
                        ...
                        <div className='flex items-center gap-2'>
                          {isCorrectOption && (
                            <span className='inline-flex items-center gap-1 px-2 py-1 bg-[#FFEBB7] border border-[#FFC84D] rounded-lg text-xs font-semibold text-[#B27A00]'>
      ...
      <div className='mt-8 flex justify-center'>
        <Link to={`/documents/${data.document?._id}`}>
          <button className='group relative px-8 h-12 bg-linear-to-r from-[#FFAE00] to-[#E69D00] hover-[#B27A00] font-semibold text-sm rounded-xl transition-all duration-200 shadow-lg shadow-[#FFAE00]/25 active:scale-95 overflow-hidden'>
