    ...
    <div className='max-w-4xl mx-auto'>
      ...
      <div className='mb-6'>
        ...
        <div className='relative h-2 bg-slate-100 rounded-full overflow-hidden'>
          <div
            className='absolute inset-y-0 left-0 bg-linear-to-br from-[#FFC84D] to-[#FFAE00] rounded-full transition-all duration-500 ease-out'
      ...
      <div className='bg-white/80 backdrop-blur-xl border-2 border-slate-200 rounded-2xl shadow-xl shadow-slate-200/50 p-6 mb-8'>
        <div className='inline-flex items-center gap-2 px-4 py-2 bg-linear-to-br from-[#FFF4D6] to-[#FFEBB7] border border-[#FFC84D] rounded-xl mb-6'>
          <div className='w-2 h-2 bg-[#FFAE00] rounded-full animate-pulse' />
            ...
            return (
              <label
                ...
                className={`group relative flex items-center p-3 border-2 rounded-xl cursor-pointer transition-all duration-200
                  ${isSelected
                    ? 'border-[#FFAE00] bg-[#FFF4D6] shadow-lg shadow-[#FFAE00]/10'
                ...
                <div className={`shrink-0 w-5 h-5 rounded-full border-2 transition-all duration-200
                        ${isSelected
                    ? 'border-[#FFAE00] bg-[#FFF4D6]'
                    : 'border-slate-300 bg-white group-hover:border-[#FFC84D]'
                  }`}>
                  {isSelected && (
                ...
                <span className={`ml-4 text-sm font-medium transition-colors duration-200 
                          ${isSelected ? 'text-[#593D00]' : 'text-slate-700 group-hover:text-slate-900'
                  }`}>
                  {option}
                </span>
                ...
                {isSelected && (
                  <CheckCircle2
                    className='ml-auto w-5 h-5 text-[#FFAE00]'
                    strokeWidth={2.5} />
                )}
              </label>
            );
      ...
      <div className='flex items-center justify-between gap-4'>
        ...
        {currentQuestionIndex === quiz.questions.length - 1 ? (
          <button
            ...
            disabled={submitting}
            className='group relative px-8 h-12 bg-linear-to-r from-[#FFAE00] to-[#E69D00] hover:from-[#E69D00] hover:to-[#B27A00] font-semibold text-sm rounded-xl transition-all duration-200 shadow-lg shadow-[#FFAE00]/25 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed disabled:active:scale-100 overflow-hidden'>
          ...
          return (
            <button
              ...
              className={`w-8 h-8 rounded-lg font-semibold text-xs transition-all duration-200
                ${isCurrent
                  ? 'bg-linear-to-r from-[#FFAE00] to-[#E69D00] shadow-lg shadow-[#FFAE00]/25 scale-110'
                  : isAnsweredQuestion
                    ? 'bg-[#FFEBB7] text-[#B27A00] hover:bg-[#FFDD8A]'
